...
Develop a locally hosted Python application that analyzes a content draft and scores it against 17 predefined sub-parameters using a Hybrid (Regex + LLM) architecture. The primary goal is maximum scoring objectivity and granularity, ensuring content meets the "Challenger" standard.

### 1.2 Goals for MVP

  * Implement the 17 isolated scoring sub-parameters (defined in 3.3).
  * Utilize the **"Granular Parallel" Architecture**: Execute 16 asynchronous LLM API calls (plus 1 internal Regex check) to evaluate 17 total sub-parameters simultaneously to eliminate the "Halo Effect."
...
graph TD
    ...
    C --> B;
    B --> D(Layer 2: Async Fan-Out - 16 Calls via LLMClient.py);

    subgraph Parallel Execution (16 Isolated LLM Agents)
        D -- Parallel --> E1(Agent 1A);
        D -- Parallel --> E2(Agent 1B);
        D -- Parallel --> E3(...);
        D -- Parallel --> E16(Agent 5B);
    end

    E1 & E2 & E3 & E16 --> F(LLM API Provider);
...
### 3.2 Layer 2 (LLM Client & Agents)
  ...
  * **Function:** Executes 16 specialized prompts against the LLM API asynchronously.
  * **Model:** Claude Sonnet 4.5 (`claude-sonnet-4-5-20250929`)
  * **Requirements:**
      * **Parallel Execution:** Uses `asyncio.gather` to run all 16 agents concurrently.
...
### 3.3 The 17 Sub-Parameters & Strategic Weighting

The weighting system is designed to prioritize the "Challenger Voice" (P1: 30%) and "Strategic Value" (P4: 30%), ensuring substance matches style. P3 (Clarity/BLUF) remains critical at 25%.
...
#### P3: Structural Integrity & Clarity (25% Total | 4 LLM Calls)

*Rationale: Emphasizes BLUF and "fluff-free" writing mandated by the style guide and demanded by Sales Leaders.*

  * **3A: BLUF (10%):** Bottom Line Up Front (conclusion-comparison Chain-of-Thought logic).
  * **3C: Conciseness & Human Language (5%):** Fluff-free, no jargon/nominalization. Combines conciseness with accessible language.
  * 3B: Scannability & Hierarchy (5%): Skimmable headers, no walls of text.
  * 3D: Specificity (5%): Concrete, quantified claims.

#### P2: Brand Hygiene & Compliance (5% Total | 1 Regex, 2 LLM Calls)

*Rationale: Minimal weight (5%), but absolute Veto Power (Gate 3). Any violation triggers publish failure. All 3 sub-parameters require contextual analysis.*

  * 2A: Mechanical Compliance (2%): (Layer 1 Regex) `monday.com` lowercase, Oxford comma, %, Sub-items.
  * **2B: Contextual Terminology (2%):** (Layer 2 LLM) Referent-based logic for "Tool/Hub/Task" - fails only when referring to monday.com.
  * 2C: Persona & Lexicon (1%): (Layer 2 LLM) Customers (not Users); Manager (not Boss); No ad hominem descriptors (crazy/insane).

#### P4: Strategic Value & Depth (30% Total | 4 LLM Calls)

*Rationale: **Doubled to 30%** to ensure substance equals style. Prioritizes explicit utility for Sales Leaders and combats generic AI output.*

  * **4A: Audience Alignment (10%):** Explicitly for Sales/Revenue Leaders (not generic business advice).
  * **4B: Actionability (10%):** Implementable today - the "Monday Morning Test."
  * 4C: Evidence & Examples (5%): Data-backed claims, real scenarios.
  * 4D: Originality & AI Detection (5%): Fresh perspective; avoids predictable AI patterns.

#### P5: Engagement & Discoverability (10% Total | 2 LLM Calls)

*Rationale: Important for reach, but secondary to core quality. Prioritizes initial attention and SEO.*

  * **5A: Headline & Hook (5%):** Compelling title; empathy hook in intro.
  * 5B: SEO & Shareability (5%): Keyword naturalism; status signal (worth sharing).
...
### 3.4 Scorer & Decision Engine
  ...
  * **Two-Level Aggregation:**
    ...
    2.  **Level 2:** Calculate the final score based on the weighted average of the 5 Parameters (P1: 30%, P4: 30%, P3: 25%, P5: 10%, P2: 5%).
  * **Logic (The 3-Gate System):** Determines "Publish-Ready" status (True/False).
    ...
    3.  **Gate 3:** Zero Critical Violations in ANY Brand Hygiene sub-parameter (2A, 2B, or 2C) (The Brand Veto; non-negotiable).
...
The `/prompts` directory requires 16 distinct files.
...
│   ├── 1A_Positive.txt
│   ├── 1B_Direct.txt
│   // ... (16 files total, excluding 2A Regex)
│
├── /src
│   ├── main.py (CLI entry point)
│   ├── orchestrator.py
│   ├── config.py (Weights for all 17 sub-parameters, Thresholds)
│   ├── regex_checker.py
│   ├── llm_client.py (Handles 16 Async API calls)
...
## Phase 2: Prompt Engineering (4.0 Hours) - Parallelizable
  ...
  * *CRITICAL PATH: This phase is extensive due to the 16 isolated prompts.*

<!-- end list -->

1.  [ ] **(User/Dev) Write the 16 LLM Prompts:** Draft all prompts and save them in the `/prompts` directory.
      * [ ] P1 (Tone) - 4 Prompts
      * [ ] P2 (Brand) - 2 Prompts (Ensure 2B includes the "Tool Paradox" logic).
      * [ ] P3 (Structure) - 4 Prompts (Ensure 3A includes the "BLUF" Chain-of-Thought logic).
      * [ ] P4 (Value) - 4 Prompts
      * [ ] P5 (Engagement) - 2 Prompts
...
2.  [ ] **(Dev) LLM Client:** Implement `src/llm_client.py`.
      * Must handle the 16 asynchronous API calls efficiently (manage concurrency/rate limits if necessary).
...
3.  [ ] **(User/Dev) Prompt Refinement Loop:** Analyze the feedback provided by *each* of the 16 LLM agents. Refine prompts where feedback is weak or scores misalign with human intuition.
