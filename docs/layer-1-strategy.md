# Developer Implementation Document: Layer 1 (The Hard Gate)

## 1.0 Overview

**Objective:** Implement the deterministic content validation layer (`Layer 1: The Hard Gate`).

**File:** `src/regex_checker.py`

**Strategy (The "Zero Ambiguity" Principle):** This layer checks the input text synchronously for violations that are incorrect 100% of the time, regardless of context. It must have zero tolerance for false positives.

**Veto Power (Gate 3):** Any flag generated by Layer 1 (regardless of severity) means the content fails Gate 3 and cannot be marked as "Publish-Ready."

## 2.0 Core Logic and Output

The implementation requires a function `run_layer_1_checks` that accepts the raw content draft (string) and returns a list of identified violations (`Flags`).

### 2.1 Execution Flow

1.  Initialize an empty list for `flags`.
2.  Iterate through the `LAYER_1_RULES` dictionary.
3.  Use `re.finditer()` to find all occurrences of the `pattern` in the content draft.
4.  **Context Extraction:** For each match, extract the exact violation text AND the surrounding text snippet (the `context`) using the match indices. This is crucial for the editor to locate the error.
5.  Create a `Flag` object and append it to the list.

### 2.2 The `Flag` Output Object

The output must align with the PRD requirements (PRD 5.0) and provide detailed information for the final JSON report.

```python
# Expected structure of a single Flag object
{
    "source": "Layer 1 (Regex)",
    "rule_id": "MONDAY_COM_CASE",       # The key from the rules dictionary (for debugging)
    "severity": "Critical",             # Taken from the rules dictionary
    "message": "...",                   # Taken from the rules dictionary
    "violation": "Monday.com",          # The exact text that matched the regex
    "context": "...At the start, Monday.com helps teams..." # The surrounding snippet
}
```

## 3.0 Finalized Implementation (`src/regex_checker.py`)
